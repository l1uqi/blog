...
项目 Map 解决的是“AI 应该先去哪里看”的问题。它不是完整文档，也不是规则全集，而是一张给 AI 的项目导航图。

```text
任务来了
  ↓
Project Map 告诉 AI 入口在哪里
  ↓
AI 找到相关模块、规则、风险区域
  ↓
再按任务加载具体 Context
...
一张有用的项目 Map，至少应该回答四类问题：

| 问题 | Map 里应该有什么 |
| --- | --- |
| 这个项目分成哪些区域 | 模块列表和职责边界 |
| 常见任务从哪里入口 | 新增接口、改页面、查权限等入口线索 |
| 哪些地方风险高 | 公共组件、兼容逻辑、历史包袱 |
| 规则散落在哪里 | Rules、Skills、Memory 的索引 |
